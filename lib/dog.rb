require 'pry'

class Dog
  attr_accessor :name, :breed, :id
  # attr_reader :id

  def initialize(name:name, breed:breed, id:nil)
    @id = id
    @name = name
    @breed = breed
  end

  def self.create_table
    table_check_sql = <<-SQL
      SELECT tbl_name FROM sqlite_master WHERE type='table' AND tbl_name='dogs';
    SQL
    if table_check_sql = nil
      sql = <<-SQL
        CREATE TABLE dogs (
          id INTEGER PRIMARY KEY,
          name TEXT,
          breed TEXT);
      SQL
      DB[:conn].execute(sql)
    end
  end

  def self.drop_table
    sql = <<-SQL
      DROP TABLE dogs
    SQL
    DB[:conn].execute(sql)
  end

  def update
    sql = <<-SQL
      UPDATE dogs SET name=?, breed=? WHERE id=?
    SQL
    DB[:conn].execute(sql, self.name, self.breed, self.id)
  end

  def save
    if self.id
      self.update
    else
      dog = Dog.new(name:name, breed:breed)
      sql = <<-SQL
        INSERT INTO dogs(name, breed) VALUES (?, ?)
      SQL
      DB[:conn].execute(sql, self.name, self.breed)
      dog.id = DB[:conn].execute("SELECT last_insert_rowid() FROM dogs")[0][0]
    end
    dog
  end

  def self.create(name:, breed:)
    dog = self.new(name:name, breed:breed)
    dog.save
  end

  def self.find_by_id(num)
    sql = <<-SQL
      SELECT * FROM dogs WHERE id=?
    SQL
    result = DB[:conn].execute(sql, num).flatten
    dog = Dog.new(name:result[1], breed:result[2], id:result[0])
    dog
  end

  def self.find_or_create_by(name:, breed:)
    dog = DB[:conn].execute("SELECT * FROM dogs WHERE name =? AND breed =?", name, breed)
     if !dog.empty?
       dog_data = dog[0]
       dog = self.new(name: dog_data[1], breed: dog_data[2], id:dog_data[0])
     else
       dog = self.create(name: name, breed: breed)
     end
     dog
  end

  def self.new_from_db(row)
    new_dog = Dog.new(name:row[1], breed:row[2], id:row[0])
  end

  def self.find_by_name(x)
    sql = <<-SQL
      SELECT * FROM dogs WHERE name=?
    SQL
    result = DB[:conn].execute(sql, x).flatten
    new_dog = Dog.new(name:result[1], breed:result[2], id:result[0])
  end

end
